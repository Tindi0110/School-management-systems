...
def auto_allocate_hostel(sender, instance, created, **kwargs):
    """
    Automatically allocates a bed for boarding students if available.
    Triggered on creation or when category is changed to BOARDING.
    """
    if kwargs.get('raw'):
        return
        
    # Only proceed if Boarding and no active allocation exists
    if instance.category == 'BOARDING' and not HostelAllocation.objects.filter(student=instance, status='ACTIVE').exists():
