...
const getUrlTag = (url?: string): string | undefined => {
  ...
  if (
    ["finance", "payment", "invoice", "adjustment", "fee-structure"].some((t) =>
      urlLower.includes(t),
    )
  ) {
    return "finance";
  }
  if (["students", "parents"].some((t) => urlLower.includes(t))) {
    return "students";
  }
  if (["staff"].some((t) => urlLower.includes(t))) {
    return "staff";
  }
  if (["hostel", "room", "bed"].some((t) => urlLower.includes(t))) {
    return "hostels";
  }
  if (
    ["library", "book", "lending", "copy", "fine"].some((t) =>
      urlLower.includes(t),
    )
  ) {
    return "library";
...
api.interceptors.request.use(
  (config) => {
    ...
    if (method && ["post", "put", "patch", "delete"].includes(method)) {
      ...
      if (
        ["academics", "classes", "subjects", "exams", "term"].some((t) =>
          url.toLowerCase().includes(t),
        )
      ) {
        clearApiCache("academics");
        clearApiCache("students"); // Student performance depends on academics
      }
    }
...
export const recentWith = (extra?: Record<string, any>) => ({
  params: { page_size: 10, ...extra },
});
export const summaryWith = (extra?: Record<string, any>) => ({
  params: { page_size: 50, ...extra },
});
...
export const authAPI = {
  ...
  resetPassword: (email: string) =>
    axios.post(`${API_BASE_URL.replace("/api", "")}/api/auth/password-reset/`, {
      email,
    }),
  resetPasswordConfirm: (uidb64: string, token: string, data: any) =>
    axios.post(
      `${API_BASE_URL.replace("/api", "")}/api/auth/password-reset-confirm/${uidb64}/${token}/`,
      data,
    ),
  resendVerificationPublic: (email: string) =>
    axios.post(
      `${API_BASE_URL.replace("/api", "")}/api/auth/resend-verification/public/`,
      { email },
    ),
  verifyEmailOTP: (email: string, otp: string) =>
    axios.post(
      `${API_BASE_URL.replace("/api", "")}/api/auth/verify-email-otp/`,
      { email, otp },
    ),

  staffApproval: {
    getPending: () =>
      api.get(`${API_BASE_URL.replace("/api", "")}/api/auth/staff-approval/`),
    approve: (userId: number) =>
      api.post(
        `${API_BASE_URL.replace("/api", "")}/api/auth/staff-approval/${userId}/`,
      ),
    ...
    resendVerification: (userId: number) =>
      api.post(
        `${API_BASE_URL.replace("/api", "")}/api/auth/resend-verification/${userId}/`,
      ),
  },
};
...
export const studentsAPI = {
  getAll: (params?: Record<string, any>) =>
    api.get("students/", allWith(params)),
  ...
  minimalSearch: (params?: Record<string, any>) =>
    api.get("students/minimal_search/", allWith(params)),
  syncAllBalances: () => api.post("students/sync_all_balances/"),
  linkParent: (
    studentId: number,
    data: { phone?: string; parent_id?: number },
  ) => api.post(`students/${studentId}/link_parent/`, data),
  unlinkParent: (studentId: number, data: { parent_id: number }) =>
    api.post(`students/${studentId}/unlink_parent/`, data),

  parents: {
    getAll: (params?: Record<string, any>) =>
      api.get("parents/", allWith(params)),
    ...
    getForStudent: (studentId: number) =>
      api.get("parents/", allWith({ student_id: studentId })),
  },
  admissions: {
  ...
  discipline: {
    getAll: (studentId?: number) =>
      api.get("discipline/", allWith({ student_id: studentId })),
  ...
  health: {
    getOne: (studentId: number) =>
      api.get("health/", allWith({ student_id: studentId })),
  ...
  activities: {
    getAll: (studentId: number) =>
      api.get("activities/", allWith({ student_id: studentId })),
  ...
  documents: {
    getAll: (studentId: number) =>
      api.get("student-documents/", allWith({ student_id: studentId })),
    create: (data: any) => {
...
export const academicsAPI = {
  ...
  resources: {
    ...
    update: (id: number, data: any) =>
      api.put(`learning-resources/${id}/`, data),
    patch: (id: number, data: any) =>
      api.patch(`learning-resources/${id}/`, data),
  ...
  syllabus: {
    ...
    update: (id: number, data: any) =>
      api.put(`syllabus-coverage/${id}/`, data),
    patch: (id: number, data: any) =>
      api.patch(`syllabus-coverage/${id}/`, data),
  ...
  studentSubjects: {
    ...
    patch: (id: number, data: any) =>
      api.patch(`student-subjects/${id}/`, data),
...
export const financeAPI = {
  ...
  invoices: {
    ...
    generateBatch: (data: {
      ...
      term: number;
      year_id: number;
    }) => api.post("invoices/generate_batch/", data),
    syncAll: () => api.post("invoices/sync_all/"),
    dashboardStats: (params?: any) =>
      api.get("invoices/dashboard_stats/", { params }),
    sendReminders: (data: {
      selected_ids: number[];
...
export const hostelAPI = {
  hostels: {
    getAll: (params?: Record<string, any>) =>
      api.get("hostels/", allWith(params)),
  ...
  allocations: {
    getAll: (params?: Record<string, any>) =>
      api.get("hostel-allocations/", allWith(params)),
    create: (data: any) => api.post("hostel-allocations/", data),
    update: (id: number, data: any) =>
      api.put(`hostel-allocations/${id}/`, data),
    delete: (id: number) => api.delete(`hostel-allocations/${id}/`),
    transfer: (id: number, newBedId: number) =>
      api.post(`hostel-allocations/${id}/transfer/`, { new_bed_id: newBedId }),
  },
  attendance: {
    getAll: (params?: Record<string, any>) =>
      api.get("hostel-attendance/", allWith(params)),
    create: (data: any) => api.post("hostel-attendance/", data),
    update: (id: number, data: any) =>
      api.put(`hostel-attendance/${id}/`, data),
  ...
  discipline: {
    getAll: (params?: Record<string, any>) =>
      api.get("hostel-discipline/", allWith(params)),
    create: (data: any) => api.post("hostel-discipline/", data),
    update: (id: number, data: any) =>
      api.put(`hostel-discipline/${id}/`, data),
    delete: (id: number) => api.delete(`hostel-discipline/${id}/`),
  },
  assets: {
    getAll: (params?: Record<string, any>) =>
      api.get("hostel-assets/", allWith(params)),
  ...
  guests: {
    getAll: (params?: Record<string, any>) =>
      api.get("hostel-guests/", allWith(params)),
  ...
  maintenance: {
    getAll: (params?: Record<string, any>) =>
      api.get("hostel-maintenance/", allWith(params)),
    create: (data: any) => api.post("hostel-maintenance/", data),
    update: (id: number, data: any) =>
      api.put(`hostel-maintenance/${id}/`, data),
...
export const libraryAPI = {
  config: {
    getAll: (params?: Record<string, any>) =>
      api.get("library-config/", allWith(params)),
    create: (data: any) => api.post("library-config/", data),
  },
  books: {
    getAll: (params?: Record<string, any>) =>
      api.get("books/", allWith(params)),
  ...
  copies: {
    getAll: (params?: Record<string, any>) =>
      api.get("book-copies/", allWith(params)),
  ...
  lendings: {
    ...
    extendDueDate: (id: number, data: { days: number }) =>
      api.post(`book-lendings/${id}/extend_due_date/`, data),
  },
  fines: {
    getAll: (params?: Record<string, any>) =>
      api.get("library-fines/", allWith(params)),
  ...
  reservations: {
    getAll: (params?: Record<string, any>) =>
      api.get("book-reservations/", allWith(params)),
...
export const transportAPI = {
  ...
  pickupPoints: {
    getAll: (params?: Record<string, any>) =>
      api.get("pickup-points/", allWith(params)),
  ...
  allocations: {
    getAll: (params?: Record<string, any>) =>
      api.get("transport-allocations/", allWith(params)),
    create: (data: any) => api.post("transport-allocations/", data),
    update: (id: number, data: any) =>
      api.put(`transport-allocations/${id}/`, data),
    delete: (id: number) => api.delete(`transport-allocations/${id}/`),
  },
  tripLogs: {
    getAll: (params?: Record<string, any>) =>
      api.get("trip-logs/", allWith(params)),
    ...
    markAttendance: (tripId: number, studentId: number, isPresent: boolean) =>
      api.post(`trip-logs/${tripId}/mark_attendance/`, {
  ...
  maintenance: {
    getAll: (params?: Record<string, any>) =>
      api.get("vehicle-maintenance/", allWith(params)),
    create: (data: any) => api.post("vehicle-maintenance/", data),
    update: (id: number, data: any) =>
      api.put(`vehicle-maintenance/${id}/`, data),
    delete: (id: number) => api.delete(`vehicle-maintenance/${id}/`),
  },
  fuel: {
    getAll: (params?: Record<string, any>) =>
      api.get("fuel-records/", allWith(params)),
  ...
  incidents: {
    getAll: (params?: Record<string, any>) =>
      api.get("transport-incidents/", allWith(params)),
    create: (data: any) => api.post("transport-incidents/", data),
    update: (id: number, data: any) =>
      api.put(`transport-incidents/${id}/`, data),
